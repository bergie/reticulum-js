...
import { Destination, DestinationType, Direction } from '../../src/core/destination.js';
...
async function testDestination() {
    ...
    const singleDest = await Destination.SINGLE("myapp", Direction.OUT, myIdentity);
    ...
    const groupDest = await Destination.GROUP("mygroup", Direction.OUT, myIdentity);
    ...
    // Test IN/OUT helpers
    const inDest = await Destination.IN("myapp", DestinationType.SINGLE, myIdentity);
    assert.strictEqual(inDest.direction, Direction.IN);
    assert.ok(inDest.destinationHash);

    const outDest = await Destination.OUT("myapp", DestinationType.SINGLE, myIdentity);
