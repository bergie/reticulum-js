...
- The generic LXMF-style proof-of-work stamp primitives (`stampWorkblock`,
...
- `LinkStatus` is now exported from the package root (alongside `Link`) so
  dependent packages don't need a deep `src/transport/link.js` import.

### Removed
- **Breaking:** the `@reticulum/core/src/lxmf/*` and `@reticulum/core/src/rfed/*`
