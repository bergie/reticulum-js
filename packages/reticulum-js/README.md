...
// TCP and shared-instance interfaces are Node-only — install `reticulum-js-node`.
import {
...
[`reticulum-js-node`](../reticulum-js-node/README.md) companion.
...
| `webrtc.js` (`WebRTCInterface`) | ✅ | inject a `createPeerConnection` factory (see [`reticulum-js-webrtc-node`](../reticulum-js-webrtc-node/README.md) for Node) |

**Node-only (in [`reticulum-js-node`](../reticulum-js-node/README.md)):**
...
```js
import { LocalClientInterface } from "reticulum-js-node";
