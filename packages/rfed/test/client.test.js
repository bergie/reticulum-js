...
import { Allow, Destination } from "@reticulum/core/src/core/destination.js";
...
import { deliveryHashFor } from "../src/channel.js";
...
describe("rfed client — subscribe + publish + live fanout receive", () => {
  ...
  test("a publish without a valid stamp is silently dropped", async () => {
    ...
    const { deriveChannel } = await import("../src/channel.js");
...
describe("rfed client — pull paging", () => {
  test("pull drains deferred blobs for a channel", async () => {
    ...
    const { deriveChannel } = await import("../src/channel.js");
    const { unwrapChannelMessage } = await import("../src/blob.js");
