 ...
 * @description Enforces SPEC.md §1.1 for `@reticulum/rfed`: the browser
 ...
 *   the workspace dependencies on `@reticulum/core` and `@reticulum/lxmf`
 *   (both themselves WinterTC-pure).
 *
...
/**
 * Bare specifiers allowed in the browser graph: the workspace dependencies on
 * the core RNS stack and the LXMF messaging package (including subpaths),
 * both of which enforce the same WinterTC-pure boundary themselves.
 * @type {Set<string>}
 */
const ALLOWED_BARE = new Set(["@reticulum/core", "@reticulum/lxmf"]);
...
test("src/index.js import graph has no Node.js dependencies (SPEC.md §1.1)", () => {
  ...
  assert.equal(
    ...
    "The browser import path must not import third-party (bare) specifiers — " +
      "only the @reticulum/* workspace dependencies are allowed:\n" +
      bare.map(fmt).join("\n"),
  );
  assert.equal(
    unresolved.length,
