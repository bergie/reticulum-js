...
### Changed
- **core**: `Destination.knownDestinations` entries are now named `KnownDestination`
...
- **core**: Removed leftover conversational/debug comments and logging (a
...
- **core**: An announce payload that is too small to be a valid body (now a thrown
...
- **rfed**: `RFedNode`'s `/rfed/pull` handler now answers an unidentified caller with
...
- **rfed**: `RFedClient.pull()` now throws on a numeric (≥ `0xF0`) node response
  instead of silently returning an empty result, so callers can re-identify
  on a fresh link.
### Fixed
- **core**: Ingress-control frequency sampling (`receivedAnnounce`,
...
- **core**: **rfed auto-sync is now testable end-to-end.** New `test/rfed/fed_sync.test.js`
  ...
  noted in 0.6.2.
### Changed
- **node**: **`rfed` CLI now uses the `FedSync` auto-sync engine** instead of a manual
...
### Fixed
- **core**: **JSR documentation score: module docs & symbol coverage.** The `@reticulum/core`
...
- **core**: **rfed `FedSync` now honours `fromStaticOnly`** (mirrors Rust
...
- **core**: **`RFedNode.start()` now seeds static peers and sets the local node hash**
